...
//
//interface HopperLogic : BlockWithEnchantment {
//
...
//    companion object {
//
//        fun extract(world: World, hopper: Hopper, info: CallbackInfoReturnable<Boolean>) {
//            if (preventsExtract(world, hopper.hopperX, hopper.hopperY + 1.0, hopper.hopperZ)) {
//                info.returnValue = false
//            }
//        }
//
//        fun insertAndExtract(hopper: HopperBlockEntity) {
//            if (hopper is HopperLogic) {
//                val cooldown =
//                    hopper.transferCooldown - if (hopper.hasEnchantments()) ((hopper.transferCooldown - 2) / 5.0 * hopper.efficiency).toInt() else 0
//                hopper.setHopperCooldown(max(1, cooldown))
//            }
//        }
//
//        fun filterIsInventoryEmpty(inventory: Inventory, facing: Direction): Boolean {
//            if (inventory is HopperLogic) {
//                val isFilter = ((inventory as? HopperLogic)?.silkTouch ?: 0) > 0
...
//            val isFilter = ((inventory as? HopperLogic)?.silkTouch ?: 0) > 0
//            val slots =
//                if (inventory is SidedInventory) IntStream.of(*inventory.getAvailableSlots(facing)) else IntStream.range(
//                    0,
//                    inventory.size()
//                )
//            return slots.filter { !(inventory.getStack(it).isEmpty || (isFilter && inventory.getStack(it).count == 1)) }
//        }
//
//        fun redstoneEnabled(
//            world: World,
...
//                    }
//
//                    if (canMergeItems(outputStack, stackFromInv)) {
