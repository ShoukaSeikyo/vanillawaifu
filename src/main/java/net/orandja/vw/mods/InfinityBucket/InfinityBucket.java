package net.orandja.vw.mods.InfinityBucket;
...
public interface InfinityBucket {
    ...
    static void beforeLaunch() {
        ...
        DispenserBlock.registerBehavior(Items.BUCKET, (pointer, stack) -> {
            BlockPos blockPos;
            BlockState state = pointer.getWorld().getBlockState(blockPos = pointer.getPos().offset(pointer.getBlockState().get(DispenserBlock.FACING)));
            Block block = state.getBlock();
            if(!(block instanceof FluidDrainable fluidBlock) || fluidBlock.tryDrainFluid(pointer.getWorld(), blockPos, state).isEmpty()) {
                return fallbackBehavior.dispenseSilently(pointer, stack);
            }

            pointer.getWorld().emitGameEvent(null, GameEvent.FLUID_PICKUP, blockPos);
            if(isInfinity(stack)) {
                return stack;
            }
            ...
            if(pointer.getBlockEntity() instanceof DispenserBlockEntity dispenser && dispenser.addToFirstFreeSlot(new ItemStack(item)) < 0) {
