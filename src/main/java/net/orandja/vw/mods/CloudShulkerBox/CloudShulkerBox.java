package net.orandja.vw.mods.CloudShulkerBox;
...
import net.orandja.mcutils.StackUtils;
...
public interface CloudShulkerBox {
    ...
    class CloudShulkerBoxRecipe extends ShapelessRecipe {
        ...
        public CloudShulkerBoxRecipe(Identifier identifier, String group, ItemStack output, DefaultedList<Ingredient> input) {
            super(identifier, group, output, input);
        }
        ...
        public boolean isOwned(Entity entity) {
            if(!isPublic() && entity != null) {
                ...
                if(TntEntity.class.isAssignableFrom(entity.getClass())) {
                    return isOwned(((TntEntity)entity).getCausingEntity());
                }
            }
            return false;
        }
    ...
    static void beforeLaunch() {
        ...
        CustomRecipe.customShapelessRecipes.put(new Identifier("vanillawaifu", "cloudbox"), CloudShulkerBoxRecipe::new);
    }
    ...
    interface BiStacksSupplier {
        List<ItemStack> supply(BlockState state, LootContext.Builder builder);
    }

    default void lootCloud(BiStacksSupplier getStacks, BlockState state, LootContext.Builder builder, CallbackInfoReturnable<List<ItemStack>> info) {
        if(builder.getNullable(LootContextParameters.BLOCK_ENTITY) instanceof CloudShulkerBox cloudBox) {
