...
public class Settings {
    ...
    public static abstract class Custom<T extends Custom.Deserializer> extends Abstract<Custom<T>, T> {

        public Custom(String path, T defaultValue) {
    ...
    public static void load(String filename) {
        FILE_NAME = filename;
        try {
            JsonObject mainNode = JsonParser.parseString(FileUtils.readFileToString(new File("config/" + FILE_NAME + ".config"), StandardCharsets.UTF_8)).getAsJsonObject();
    ...
    public static void save() {
        ...
        settings.forEach(setting -> {
            System.out.println(setting.getPath() + "; " + setting.serialize());
            if (!setting.isTemporary()) {
                mainNode.addProperty(setting.getPath(), setting.serialize());
            }
        });
        ...
        try {
            ...
            FileUtils.write(new File("config/" + FILE_NAME + ".config"), writer.toString(), StandardCharsets.UTF_8);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
